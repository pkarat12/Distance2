...
<form:form method="POST" action="${pageContext.request.contextPath}/director/send-password-to-user${emailUserModel.id}"  modelAttribute="emailUserModel">
<div class="col-md-8">
	<nav aria-label="breadcrumb">
  		<ol class="breadcrumb menu-ul-nothover">
  			...
  			<c:choose>
  				<c:when test="${role eq rol}">
  					<li class="breadcrumb-item"><a href="/director/profile-teacher${emailUserModel.id}">ДЕТАЛІ</a></li>
  				</c:when>
  				<c:otherwise>
  					<li class="breadcrumb-item"><a href="/director/profile-student${emailUserModel.id}">ДЕТАЛІ</a></li>
  				</c:otherwise>
  			</c:choose>
  			
    		<li class="breadcrumb-item active" aria-current="page">НАДІСЛАТИ ПАРОЛЬ</li>
  		</ol>
	</nav>
