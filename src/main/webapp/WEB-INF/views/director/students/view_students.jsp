...
<div >
	<div class="form-inline justify-content-between">
		<div class="form-group">
			<a href="${pageContext.request.contextPath}/director/add_student" class="btn btn-info buttonsAll">
				<span class="glyphicon glyphicon-plus"></span> Додати </a>
		</div>	
		<div class="form-group pull-right">
			<div class="form-group">
				<form:form action="${pageContext.request.contextPath}/director/search/students/1" method="post" class="seachAndAdd" modelAttribute="filterModel">
					<div class="form-group">
			...
			<div class="form-group">
				<form:form action="${pageContext.request.contextPath}/director/students/1" method="get" class="form-inline">
					<button type="submit" class="btn btn-default buttonsAll">
...
<div>
	<div class="table-responsive" >
		<table class="table table-hover table-bordered table-condensed" >
			...
			<tbody>
				<c:forEach items="${studentListModel}" var="one">
					<tr onclick="openWin('${pageContext.request.contextPath}/director/profile-student${one.id}')">
						<td class="borderTable">${one.lastName}</td>
						<td class="borderTable">${one.firstName}</td>
						<td class="borderTable">${one.middleName}</td>
						<td class="borderTable">${one.classStudent}</td>
					</tr>
				</c:forEach>
			</tbody>
	</table>
</div>
<c:if test="${flag and studentsList.totalPages > 1}">
	<div class="text-center">
		<c:url var="firstUrl" value="${pageContext.request.contextPath}/director/students/1" />
		<c:url var="lastUrl" value="${pageContext.request.contextPath}/director/students/${studentsList.totalPages}" />
		<c:url var="prevUrl" value="${pageContext.request.contextPath}/director/students/${currentIndex-1}" />
		<c:url var="nextUrl" value="${pageContext.request.contextPath}/director/students/${currentIndex+1}" />
			<ul class = "pagination pagination-sm">
				<c:choose>
					...
					<c:forEach var="i" begin="${beginIndex}" end="${endIndex}">
						<c:url var="pageUrl" value="${pageContext.request.contextPath}/director/students/${i+1}" />
							<c:choose>
				...
				<c:choose>
					<c:when test="${currentIndex == studentsList.totalPages}">
						<li class="page-item"><a class="menu-ul" href="#">&gt;</a></li>
						<li class="page-item"><a class="menu-ul" href="#">&gt;&gt;</a></li>
				</c:when>
					<c:otherwise>
