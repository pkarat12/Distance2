...
<div >
	<div class="form-inline justify-content-between">
		<div class="form-group">
			<a href="${pageContext.request.contextPath}/teacher/course/add" class="btn btn-info buttonsAll">
				<span class="glyphicon glyphicon-plus"></span> Додати </a>
		</div>	
		<div class="form-group pull-right">
			<div class="form-group">
					<form:form action="${pageContext.request.contextPath}/teacher/courses/search/1" method="get" class="form-inline" modelAttribute="filterModel">
					<div class="form-group">
						<form:select class="form-control selectpicker input_border" data-style="btn-primary" path="grade">
							<option value="${null}">Всі роки</option>
								<c:forEach items="${gradeModel}" var="grade">
									<option value="${grade}">${grade.title}</option>
								</c:forEach>
						</form:select>
					</div>
					<div class="form-group">
			...
			<div class="form-group">
				<form:form action="${pageContext.request.contextPath}/teacher/courses/1" method="get" class="form-inline">
					<button type="submit" class="btn btn-default buttonsAll">
	...
	<div class="table-responsive" >
		<table class="table table-hover table-bordered table-condensed" >
			<thead class="menu-ul-nothover">
				<tr>
					<td style="width:30%">Заголовок</td>
					<td style="width:50%">Опис</td>
					<td style="width:20%">Навчальий рік</td>
				</tr>
			</thead> 
			<tbody>
				<c:forEach items="${courseList}" var="one">
					<tr onclick="openWin('${pageContext.request.contextPath}/teacher/course/profile${one.idCourse}/1')">
						<td class="borderTable">${one.title}</td>
						<td class="borderTable">${one.description}</td>
						<td class="borderTable">${one.grade.title}</td>
					</tr>
				</c:forEach>
			</tbody>
	</table>
...
<c:if test="${flag and pageModel.totalPages >1}">
	<div class="text-center">
		<c:url var="firstUrl" value="${pageContext.request.contextPath}/teacher/courses/1" />
		<c:url var="lastUrl" value="${pageContext.request.contextPath}/teacher/courses/${pageModel.totalPages}" />
		<c:url var="prevUrl" value="${pageContext.request.contextPath}/teacher/courses/${currentIndex-1}" />
		<c:url var="nextUrl" value="${pageContext.request.contextPath}/teacher/courses/${currentIndex+1}" />
			<ul class = "pagination pagination-sm">
				<c:choose>
					...
					<c:forEach var="i" begin="${beginIndex}" end="${endIndex}">
						<c:url var="pageUrl" value="${pageContext.request.contextPath}/teacher/courses/${i+1}" />
							<c:choose>
